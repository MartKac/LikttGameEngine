...
class Wall(Actor):
  """
  Class for creating wals in the scene
  """
  ...
  def render(self, transparent=False):
    ...
    if transparent:
      print("aaa")
      self.box = self.canvas.create_rectangle(self.x, self.y, self.x+self.w, self.y+self.h, fill="", outline="")
      return
    ...
    if self.startimage is not None:
      ...
      self.box = self.canvas.create_rectangle(self.x, self.y, self.x+self.w, self.y+self.h, fill="orange", outline="")
